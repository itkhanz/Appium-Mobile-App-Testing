...
* Traversing through the generated XML in Appium Inspector can be cumbersome. To reduce the XML to important nodes:
  ...
  * Go to the source tab, and click refresh. This enables finding of the elements to be much faster as it limits
    hierarchy view to only the elements that are visible to screen.
* Alternatively you can also supply above settings as capability before starting the session:
    * add the `settings[ignoreUnimportantViews]: true` as capability and start the session.
